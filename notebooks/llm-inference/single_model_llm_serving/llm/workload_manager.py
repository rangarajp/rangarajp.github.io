...
class WorkloadManager:
    def __init__(self):
        ...
        flow("WorkloadManager", "__init__ — queues ready (batch_size=4)")
    ...
    def add_request(self, prompt: str) -> str:
        ...
        flow("WorkloadManager", f"add_request → incoming_queue  id={request_id[:8]}… prompt={prompt!r}")
        return request_id
    ...
    def add_streaming_request(self, prompt: str, client_stream, loop) -> str:
        ...
        self.sequence_map[request_id] = sequence
        flow("WorkloadManager", f"add_streaming_request → streaming_queue  id={request_id[:8]}…")
        return request_id
    ...
    def get_next_batch(self, is_streaming: bool = False) -> List[Sequence]:
        if is_streaming:
            ...
            while len(self.active_streaming_sequences) < self.batch_size and not self.incoming_streaming_queue.empty():
                ...
                pulled += 1
            if self.active_streaming_sequences and pulled:
                flow("WorkloadManager", f"get_next_batch(streaming) — pulled {pulled}, active={len(self.active_streaming_sequences)}")
            ...
            while len(self.active_sequences) < self.batch_size and not self.incoming_queue.empty():
                ...
                pulled += 1
            flow("WorkloadManager", f"get_next_batch(batch) — pulled {pulled}, active={len(self.active_sequences)}")
    ...
    def remove_finished_sequence(self, seq_id: str):
        if seq_id in self.sequence_map:
            ...
            flow("WorkloadManager", f"remove_finished_sequence id={seq_id[:8]}…")
    ...
    def update_sequence_output(self, seq_id: str, token: str, is_finished: bool = False):
        if seq_id in self.sequence_map:
            ...
            flow("WorkloadManager", f"update_sequence_output id={seq_id[:8]}… finished={is_finished} tokens={sequence.token_count}")
            return sequence
        return None
